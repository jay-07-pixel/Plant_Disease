"""EfficientNet-B3 transfer-learning classifier for plant disease detection."""
...
class EfficientNetB3TransferClassifier(nn.Module):
    """ImageNet-pretrained EfficientNet-B3 with staged transfer-learning support.

    Loads ``torchvision.models.efficientnet_b3`` with ImageNet weights, replaces
    ...
    * **Stage 2 (fine-tuning):** unfreeze the final feature stage
    ...
    def __init__(
        ...
        *,
        ...
        weights: EfficientNet_B3_Weights | None = EfficientNet_B3_Weights.IMAGENET1K_V1,
        dropout: float = 0.3,
    ) -> None:
        super().__init__()
        if num_classes < 2:
        ...
        logger.info(
            "EfficientNetB3TransferClassifier ready: classes=%d, trainable=%s, frozen=%s",
            num_classes,
            f"{counts.trainable:,}",
            f"{counts.frozen:,}",
        )
    ...
    def unfreeze_last_feature_stage(self) -> ParameterCounts:
        ...
        ResNet/DenseNet/EfficientNet-B0 experiments.
        """
        trainable_prefixes = (
            "features.7.",
            "features.8.",
...
def verify_forward_pass(
    ...
    *,
    ...
    Args:
        model: EfficientNet-B3 transfer classifier.
