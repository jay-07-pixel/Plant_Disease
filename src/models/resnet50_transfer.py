"""ResNet50 transfer-learning classifier for plant disease detection."""
...
class ResNet50TransferClassifier(nn.Module):
    """ImageNet-pretrained ResNet50 with staged transfer-learning support.

    Loads ``torchvision.models.resnet50`` with ImageNet weights, replaces ``fc``
    ...
    * **Stage 2 (fine-tuning):** unfreeze ``layer4`` and ``fc``, keep earlier
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(
            "ResNet50TransferClassifier ready: classes=%d, trainable=%s, frozen=%s",
            num_classes,
            f"{counts.trainable:,}",
            f"{counts.frozen:,}",
        )
...
def verify_forward_pass(
    ...
    *,
    ...
    Args:
        model: ResNet50 transfer classifier.
