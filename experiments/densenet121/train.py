"""DenseNet121 transfer-learning experiment orchestration."""
...
from experiments.densenet121.callbacks import EpochTimingCallback
from experiments.densenet121.config import (
    BEST_MODEL_PATH,
    ...
    DenseNet121ExperimentConfig,
    SAVE_DIR,
    TENSORBOARD_DIR,
    build_engine_config,
    build_stage_experiment,
)
from experiments.densenet121.evaluate import evaluate_on_dataloader
from experiments.densenet121.report import (
    build_epoch_records,
    build_training_report,
    save_training_report_json,
    save_training_report_markdown,
)
from src.models.densenet121_transfer import (
...
def cleanup_previous_artifacts() -> list[str]:
    """Remove artifacts from a previous DenseNet121 training run."""
    deleted: list[str] = []
...
def verify_trainer_compatibility(
    model: DenseNet121TransferClassifier,
    experiment: DenseNet121ExperimentConfig,
    *,
    batch_size: int = 4,
) -> tuple[bool, str]:
    """Verify the DenseNet121 model integrates with the existing Trainer."""
    try:
...
def verify_experiment_readiness(
    experiment: DenseNet121ExperimentConfig | None = None,
) -> dict[str, Any]:
    """Verify DenseNet121 experiment readiness without starting training."""
    experiment = experiment or DenseNet121ExperimentConfig()
    ...
    model = DenseNet121TransferClassifier(num_classes=num_classes, freeze_backbone=True)
    ...
    print_pre_training_summary(summary, title="DenseNet121 Transfer Learning — Readiness Check")
    ...
    print("Stage 2 parameter check (denseblock4 + classifier unfrozen):")
    ...
    print("  python -m experiments.densenet121.train --train")
    print()
...
def _run_stage2(
    *,
    ...
    """Run Stage 2 fine-tuning on denseblock4 and the classifier head."""
    mid_stage2_resume = (
        resume_checkpoint is not None
        and resolve_resume_start_epoch(resume_checkpoint) > experiment.stage1_epochs
    )
    ...
    if mid_stage2_resume or entering_stage2_from_resume:
        ...
        log_stage_boundary(
            event="STAGE 2 BEGINNING — Fine-tuning (denseblock4 + classifier, lower learning rate)",
            stage=2,
            details={
...
def run_two_stage_densenet121_experiment(
    experiment: DenseNet121ExperimentConfig,
    *,
    resume_checkpoint: Path | None = None,
) -> dict[str, Any]:
    """Run the full two-stage DenseNet121 transfer-learning experiment."""
    ...
    if resume_checkpoint is not None:
        ...
        print_pre_training_summary(
            summary,
            title="DenseNet121 Transfer Learning — Resume Training",
        )
        print_resume_banner(
            stage=resume_stage,
        ...
        print_pre_training_summary(
            summary,
            title="DenseNet121 Transfer Learning — Two-Stage Training Run",
        )
        forward_result = verify_forward_pass(model, device=device)
        logger.info(
...
def _finalize_experiment(
    *,
    ...
    logger.info("DenseNet121 experiment complete. Reports saved to %s and %s", REPORT_MD, REPORT_JSON)
    return report


def run_densenet121_experiment(
    experiment: DenseNet121ExperimentConfig | None = None,
) -> dict[str, Any]:
    """Train the DenseNet121 transfer classifier using two-stage transfer learning."""
    experiment = experiment or DenseNet121ExperimentConfig()
    if experiment.resume_checkpoint is not None:
        raise ValueError("Use run_densenet121_experiment_resume() to resume training.")
    return run_two_stage_densenet121_experiment(experiment)


def run_densenet121_experiment_resume(
    experiment: DenseNet121ExperimentConfig,
) -> dict[str, Any]:
    """Resume an interrupted two-stage DenseNet121 training run from a checkpoint."""
    if experiment.resume_checkpoint is None:
        raise ValueError("resume_checkpoint is required to resume training.")
    return run_two_stage_densenet121_experiment(
        experiment,
...
def main() -> None:
    """CLI entry point for the DenseNet121 transfer-learning experiment."""
    parser = argparse.ArgumentParser(description="DenseNet121 transfer-learning experiment.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Start a brand-new run and delete previous DenseNet121 artifacts. "
    ...
    if args.resume is not None:
        ...
        print()
        report = run_densenet121_experiment_resume(experiment)
    else:
        if latest_checkpoint.exists() and not args.fresh:
            message = (
                ...
                f"  python -m experiments.densenet121.train --train --resume {latest_checkpoint.as_posix()}\n\n"
                "To intentionally discard artifacts and restart:\n"
                "  python -m experiments.densenet121.train --train --fresh"
            )
            print(message)
            raise SystemExit(1)
        ...
        report = run_densenet121_experiment(experiment)
