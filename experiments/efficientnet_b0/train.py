"""EfficientNet-B0 transfer-learning experiment orchestration."""
...
from experiments.efficientnet_b0.callbacks import EpochTimingCallback
from experiments.efficientnet_b0.config import (
    BEST_MODEL_PATH,
    ...
    EfficientNetB0ExperimentConfig,
    SAVE_DIR,
    TENSORBOARD_DIR,
    build_engine_config,
    build_stage_experiment,
)
from experiments.efficientnet_b0.evaluate import evaluate_on_dataloader
from experiments.efficientnet_b0.report import (
    build_epoch_records,
    build_training_report,
    save_training_report_json,
    save_training_report_markdown,
)
from src.models.efficientnet_b0_transfer import (
...
def cleanup_previous_artifacts() -> list[str]:
    """Remove artifacts from a previous EfficientNet-B0 training run."""
    deleted: list[str] = []
...
def verify_trainer_compatibility(
    model: EfficientNetB0TransferClassifier,
    experiment: EfficientNetB0ExperimentConfig,
    *,
    batch_size: int = 4,
) -> tuple[bool, str]:
    """Verify the EfficientNet-B0 model integrates with the existing Trainer."""
    try:
...
def verify_experiment_readiness(
    experiment: EfficientNetB0ExperimentConfig | None = None,
) -> dict[str, Any]:
    """Verify EfficientNet-B0 experiment readiness without starting training."""
    experiment = experiment or EfficientNetB0ExperimentConfig()
    ...
    model = EfficientNetB0TransferClassifier(num_classes=num_classes, freeze_backbone=True)
    ...
    print_pre_training_summary(summary, title="EfficientNet-B0 Transfer Learning — Readiness Check")
    ...
    print("  python -m experiments.efficientnet_b0.train --train")
    print()
...
def run_two_stage_efficientnet_b0_experiment(
    experiment: EfficientNetB0ExperimentConfig,
    *,
    resume_checkpoint: Path | None = None,
) -> dict[str, Any]:
    """Run the full two-stage EfficientNet-B0 transfer-learning experiment."""
    ...
    if resume_checkpoint is not None:
        ...
        print_pre_training_summary(
            summary,
            title="EfficientNet-B0 Transfer Learning — Resume Training",
        )
        print_resume_banner(
            stage=resume_stage,
        ...
        print_pre_training_summary(
            summary,
            title="EfficientNet-B0 Transfer Learning — Two-Stage Training Run",
        )
        forward_result = verify_forward_pass(model, device=device)
        logger.info(
...
def _finalize_experiment(
    *,
    ...
    logger.info("EfficientNet-B0 experiment complete. Reports saved to %s and %s", REPORT_MD, REPORT_JSON)
    return report


def run_efficientnet_b0_experiment(
    experiment: EfficientNetB0ExperimentConfig | None = None,
) -> dict[str, Any]:
    """Train the EfficientNet-B0 transfer classifier using two-stage transfer learning."""
    experiment = experiment or EfficientNetB0ExperimentConfig()
    if experiment.resume_checkpoint is not None:
        raise ValueError("Use run_efficientnet_b0_experiment_resume() to resume training.")
    return run_two_stage_efficientnet_b0_experiment(experiment)


def run_efficientnet_b0_experiment_resume(
    experiment: EfficientNetB0ExperimentConfig,
) -> dict[str, Any]:
    """Resume an interrupted two-stage EfficientNet-B0 training run from a checkpoint."""
    if experiment.resume_checkpoint is None:
        raise ValueError("resume_checkpoint is required to resume training.")
    return run_two_stage_efficientnet_b0_experiment(
        experiment,
...
def main() -> None:
    """CLI entry point for the EfficientNet-B0 transfer-learning experiment."""
    parser = argparse.ArgumentParser(description="EfficientNet-B0 transfer-learning experiment.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Start a brand-new run and delete previous EfficientNet-B0 artifacts. "
    ...
    if args.resume is not None:
        ...
        print()
        report = run_efficientnet_b0_experiment_resume(experiment)
    else:
        if latest_checkpoint.exists() and not args.fresh:
            message = (
                ...
                f"  python -m experiments.efficientnet_b0.train --train --resume {latest_checkpoint.as_posix()}\n\n"
                "To intentionally discard artifacts and restart:\n"
                "  python -m experiments.efficientnet_b0.train --train --fresh"
            )
            print(message)
            raise SystemExit(1)
        ...
        report = run_efficientnet_b0_experiment(experiment)
