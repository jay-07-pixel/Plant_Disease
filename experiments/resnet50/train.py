"""ResNet50 transfer-learning experiment orchestration."""
...
from experiments.resnet50.callbacks import EpochTimingCallback
from experiments.resnet50.config import (
    BEST_MODEL_PATH,
    ...
    ResNet50ExperimentConfig,
    SAVE_DIR,
    TENSORBOARD_DIR,
    build_engine_config,
    build_stage_experiment,
)
from experiments.resnet50.evaluate import evaluate_on_dataloader
from experiments.resnet50.report import (
    build_epoch_records,
    build_training_report,
    save_training_report_json,
    save_training_report_markdown,
)
from src.models.resnet50_transfer import (
...
def cleanup_previous_artifacts() -> list[str]:
    """Remove artifacts from a previous ResNet50 training run."""
    deleted: list[str] = []
...
def build_environment_summary(
    loaders: Any,
    model: ResNet50TransferClassifier,
    *,
    device: torch.device,
    num_epochs: int,
    use_amp: bool,
...
def verify_trainer_compatibility(
    model: ResNet50TransferClassifier,
    experiment: ResNet50ExperimentConfig,
    *,
    batch_size: int = 4,
) -> tuple[bool, str]:
    """Verify the ResNet50 model integrates with the existing Trainer."""
    try:
...
def verify_experiment_readiness(
    experiment: ResNet50ExperimentConfig | None = None,
) -> dict[str, Any]:
    """Verify ResNet50 experiment readiness without starting training."""
    experiment = experiment or ResNet50ExperimentConfig()
    ...
    model = ResNet50TransferClassifier(num_classes=num_classes, freeze_backbone=True)
    ...
    print_pre_training_summary(summary, title="ResNet50 Transfer Learning — Readiness Check")
    ...
    print("  python -m experiments.resnet50.train --train")
    print()
...
def run_two_stage_resnet50_experiment(
    experiment: ResNet50ExperimentConfig,
    *,
    resume_checkpoint: Path | None = None,
) -> dict[str, Any]:
    """Run the full two-stage ResNet50 transfer-learning experiment."""
    ...
    if resume_checkpoint is not None:
        ...
        print_pre_training_summary(
            summary,
            title="ResNet50 Transfer Learning — Resume Training",
        )
        print_resume_banner(
            stage=resume_stage,
        ...
        print_pre_training_summary(
            summary,
            title="ResNet50 Transfer Learning — Two-Stage Training Run",
        )
        forward_result = verify_forward_pass(model, device=device)
        logger.info(
...
def _finalize_experiment(
    *,
    ...
    logger.info("ResNet50 experiment complete. Reports saved to %s and %s", REPORT_MD, REPORT_JSON)
    return report


def run_resnet50_experiment(
    experiment: ResNet50ExperimentConfig | None = None,
) -> dict[str, Any]:
    """Train the ResNet50 transfer classifier using two-stage transfer learning."""
    experiment = experiment or ResNet50ExperimentConfig()
    if experiment.resume_checkpoint is not None:
        raise ValueError("Use run_resnet50_experiment_resume() to resume training.")
    return run_two_stage_resnet50_experiment(experiment)


def run_resnet50_experiment_resume(
    experiment: ResNet50ExperimentConfig,
) -> dict[str, Any]:
    """Resume an interrupted two-stage ResNet50 training run from a checkpoint."""
    if experiment.resume_checkpoint is None:
        raise ValueError("resume_checkpoint is required to resume training.")
    return run_two_stage_resnet50_experiment(
        experiment,
...
def main() -> None:
    """CLI entry point for the ResNet50 transfer-learning experiment."""
    parser = argparse.ArgumentParser(description="ResNet50 transfer-learning experiment.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Start a brand-new run and delete previous ResNet50 artifacts. "
    ...
    if args.resume is not None:
        ...
        print()
        report = run_resnet50_experiment_resume(experiment)
    else:
        if latest_checkpoint.exists() and not args.fresh:
            message = (
                ...
                f"  python -m experiments.resnet50.train --train --resume {latest_checkpoint.as_posix()}\n\n"
                "To intentionally discard artifacts and restart:\n"
                "  python -m experiments.resnet50.train --train --fresh"
            )
            print(message)
            raise SystemExit(1)
        ...
        report = run_resnet50_experiment(experiment)
